...
self.addEventListener('fetch', (ev) => {
  ...
  const selfUrl = new URL(self.location);
  const isExternal = ev.request.mode == 'cors' || selfUrl.hostname !== url.hashname;

  if(isOnline){
    ev.respondWith(cacheFirst(ev))
  }else{
    ev.respondWith(fakeServerError(ev))
  }
});
...
function staleWhileRevalidate(ev) {
  ...
  return caches.match(ev.request).then((cacheResponse) => {
    const fetchResponse = fetch(ev.request).then(response => {
